...
function Build-GameWasm {
    ...
    $sourceFiles = @(
        ...
        "public/src/wasm/managers/wolves/StateMachine.cpp",
        "public/src/wasm/managers/ArmManager.cpp",
    ...
    $exportedFunctions = @(
        ...
        "_get_skeleton_joint_count", "_get_skeleton_joint_x", "_get_skeleton_joint_y",
        "_get_balance_quality", "_get_left_foot_grounded", "_get_right_foot_grounded",
        "_write_skeleton_joints_xy",
        # Wolf core (Phase 1A)
        "_get_wolf_health", "_get_wolf_state", "_get_wolf_emotion", "_get_wolf_x", "_get_wolf_y",
        # Pack basics (Phase 1B)
        "_get_pack_count", "_get_pack_plan", "_get_pack_morale",
        # Phase 2A: Terrain
        "_get_terrain_feature_count", "_get_terrain_feature_x", "_get_terrain_feature_y", "_get_terrain_feature_type",
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "Verifying required skeleton exports..." -ForegroundColor Yellow
            $null = node ./tools/scripts/verify-skeleton-exports.js ./public/wasm/game.wasm | Write-Host
            if ($LASTEXITCODE -ne 0) {
                Write-Host "Error: Required skeleton exports missing." -ForegroundColor Red
                return $false
            }
            return $true
        } else {
