...
const createWasiPreview1 = (memory) => {
	...
	// Create a plain object to satisfy SES lockdown requirements
	const wasi = Object.create(null);
	wasi.fd_write = (fd, iovPtr, iovCnt, nwrittenPtr) => {
			let written = 0
			const dataView = dv()
			const bytes = u8()
			let offset = iovPtr >>> 0
	...
	wasi.proc_exit = (code) => { throw new Error(`WASI proc_exit: ${code}`) };
	...
	wasi.clock_time_get = (_id, _precision, timePtr) => {
	...
	wasi.args_sizes_get = (argcPtr, argvBufSizePtr) => {
			const dataView = dv()
	...
	wasi.args_get = () => 0;
	wasi.environ_sizes_get = (envcPtr, envBufSizePtr) => {
			const dataView = dv()
	...
	wasi.environ_get = () => 0;
	wasi.fd_close = () => 0;
	wasi.fd_seek = (_fd, _offsetLow, _offsetHigh, _whence, newOffsetPtr) => {
			// report position 0
