...
export class ProceduralWolfSystem {
    ...
    updateWolves(deltaTime) {
        for (let [wolfId, wolf] of this.wolves) {
    ...
    updatePackDynamics(deltaTime) {
        ...
        for (let wolf of this.wolves.values()) {
        ...
        wolves.forEach((wolf, index) => {
            ...
            switch (formation) {
                ...
                case 'wedge':
                    ...
                    break
                default: // loose
                    targetOffset.x = (Math.random() - 0.5) * 6
                    targetOffset.y = (Math.random() - 0.5) * 6
                    break
            }
    ...
    render(ctx, camera) {
        ...
        const sortedWolves = Array.from(this.wolves.values()).sort((a, b) => {
            return (b.position.y + b.position.x * 0.1) - (a.position.y + a.position.x * 0.1)
        })
    ...
    destroy() {
        ...
        // Remove all wolves
        for (let wolfId of this.wolves.keys()) {
...
class WolfInstance {
    constructor(id, options = {}, systemConfig = {}) {
        ...
        this.animComponent = createEnhancedWolfAnimComponent({
            ...
            personalityTraits: options.personality || createRealisticWolfPersonality(),
            packRank: options.packRank || Math.random()
        })
...
class WolfBehaviorManager {
    ...
    setupBehaviorRules() {
        ...
        this.behaviorRules.set('pack_hunt', {
            trigger: (wolf, environment) => {
                return environment.scents.some(s => s.type === 'prey' && s.strength > 0.5) &&
        ...
        this.behaviorRules.set('territory_defense', {
            trigger: (wolf, environment) => {
                return environment.scents.some(s => s.type === 'intruder' && s.strength > 0.7) &&
        ...
        this.behaviorRules.set('rest_when_tired', {
            trigger: (wolf, environment) => {
                return wolf.animComponent.fatigueLevel > 0.8 && wolf.animComponent.speed < 0.1
            },
        ...
        this.behaviorRules.set('social_interaction', {
            trigger: (wolf, environment) => {
                return environment.packData && environment.packData.nearbyWolves.length > 1 &&
    ...
    updateWolfBehavior(wolf, environment, deltaTime) {
        ...
        for (let [name, rule] of this.behaviorRules) {
