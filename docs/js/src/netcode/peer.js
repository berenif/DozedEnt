import {all, alloc} from '../utils/utils.js'
import {ICE_TIMEOUT_MS, DATA_BUFFERED_LOW_THRESHOLD} from '../utils/constants.js'
...
export default (initiator, {rtcConfig, rtcPolyfill, turnConfig}) => {
  ...
  const setupDataChannel = channel => {
    ...
    channel.bufferedAmountLowThreshold = DATA_BUFFERED_LOW_THRESHOLD
  ...
  const waitForIceGathering = pc =>
    Promise.race([
      ...
      new Promise(res => setTimeout(res, ICE_TIMEOUT_MS))
    ]).then(() => ({
