...
void game_update(GameState* state, float deltaTime) {
    ...
    for (int i = 0; i < state->maxPlayers; i++) {
        ...
        // Keep players in bounds (normalized coordinates [0,1])
        // Convert to normalized coordinates for consistency with main WASM module
        float normalizedX = player.x / state->worldWidth;
        float normalizedY = player.y / state->worldHeight;
        
        // Apply same boundary logic as main game
        const float playerRadius = 16.0f / state->worldWidth; // Normalize radius
        
        if (normalizedX <= playerRadius) {
            normalizedX = playerRadius;
            player.vx = 0;
        }
        if (normalizedX >= 1.0f - playerRadius) {
            normalizedX = 1.0f - playerRadius;
            player.vx = 0;
        }
        if (normalizedY <= playerRadius) {
            normalizedY = playerRadius;
            player.vy = 0;
        }
        if (normalizedY >= 1.0f - playerRadius) {
            normalizedY = 1.0f - playerRadius;
            player.vy = 0;
        }
        
        // Convert back to world coordinates
        player.x = normalizedX * state->worldWidth;
        player.y = normalizedY * state->worldHeight;
