...
// Forward declarations for functions/vars defined in other headers/compilation units
...
// Spawn timer used by escalate phase to materialize density increases
static float g_escalate_spawn_timer = 0.0f;
...
static void update_escalation_phase(float dt) {
  ...
  // Materialize density increases by spawning additional enemies over time
  // Spawn cadence shortens as spawn_rate_multiplier increases
  g_escalate_spawn_timer += dt;
  float intervalBase = 6.0f; // seconds
  float interval = intervalBase / fmaxf(1.0f, g_spawn_rate_multiplier);
  if (g_escalate_spawn_timer >= interval) {
    g_escalate_spawn_timer = 0.0f;
    // Target more pressure as escalation rises, capped by MAX_ENEMIES
    unsigned int target = (unsigned int)(4u + (unsigned int)(g_escalation_level * 8.0f));
    if (target > (unsigned int)MAX_ENEMIES) target = (unsigned int)MAX_ENEMIES;
    if (g_enemy_count < target) {
      unsigned int deficit = target - g_enemy_count;
      // Spawn in small bursts, at least 1
      unsigned int toSpawn = 1u + (unsigned int)fminf(3.0f, ceilf(deficit * 0.5f));
      spawn_wolves(toSpawn);
    }
  }
}
