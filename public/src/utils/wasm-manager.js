...
import { setGlobalSeed as setVisualRngSeed } from './rng.js'
...
export class WasmManager {
  constructor() {
    ...
    this._stateCacheTimeout = 16.67; // ~1 frame at 60fps in milliseconds (reduced frequency)
    
    // Performance monitoring (simplified to reduce overhead)
  ...
  async initialize() {
    ...
    // Start memory monitoring with delay to avoid interfering with WASM loading
    if (globalMemoryOptimizer && !globalMemoryOptimizer.isMonitoring) {
      // Delay memory monitoring to avoid interfering with WASM initialization
      setTimeout(() => {
    ...
    try {
      ...
      // Use the direct WASM loader, avoiding Trystero wrapper
      ...
      if (!wasmHelperModule) {
        throw new Error('All WASM helper module paths failed. Ensure src/utils/wasm.js exists');
      }
        ...
        if (typeof this.exports.start === 'function') {
          ...
          try {
            ...
            if (typeof this.exports.set_player_input === 'function') {
              ...
              // Test update with detailed timing
              if (typeof this.exports.update === 'function') {
                console.log('🔄 Testing WASM update function...');

                // Add timeout protection to prevent infinite loop during testing
                const updateTimeout = setTimeout(() => {
                  console.error('❌ WASM test update timed out after 500ms - infinite loop detected in test');
            ...
            const startDuration = performance.now() - startTime;
            console.error(`❌ WASM start() failed after ${startDuration.toFixed(2)}ms:`, startError);
            throw startError;
          }
        } else {
          console.warn('⚠️ WASM start function not available in exports');
        }
  ...
  update(dirX, dirY, isRolling, deltaTime) {
    ...
    try {
      ...
      const updateTimeout = setTimeout(() => {
        ...
        console.error(`❌ WASM update function timed out after ${elapsed.toFixed(2)}ms - likely infinite loop`);
        console.error('WASM update inputs:', { dirX: safeDirectionX, dirY: safeDirY, isRolling: safeIsRolling, deltaTime: safeDeltaTime });
        this.isLoaded = false; // Prevent further updates
        throw new Error('WASM update timeout - infinite loop detected');
      }, 50); // 50ms timeout for game loop

      try {
        console.log('🔄 Calling WASM update with inputs:', { dirX: safeDirectionX, dirY: safeDirY, isRolling: safeIsRolling, deltaTime: safeDeltaTime });
        if (arity >= 4) {
        ...
        const updateDuration = performance.now() - updateStartTime;
        console.error(`❌ WASM update failed after ${updateDuration.toFixed(2)}ms:`, error);
        clearTimeout(updateTimeout);
        throw error;
      }
      ...
      // Only update average metrics periodically to reduce overhead
  ...
  checkWasm2Features() {
    ...
    try {
      ...
      if (typeof WebAssembly !== 'undefined' && WebAssembly.validate) {
        // Test SIMD by trying to validate a simple SIMD instruction
      ...
      // Check for bulk memory operations (memory.fill, memory.copy)
      if (this.exports && this.exports.memory) {
        features.bulkMemory = typeof this.exports.memory.fill === 'function' && 
                              typeof this.exports.memory.copy === 'function';
      }

      // Check for multiple memories support
      if (this.exports && this.exports.memory) {
        features.multipleMemories = this.exports.memory instanceof WebAssembly.Memory;
      }

      // Check for exceptions support (try-catch in WASM)
      features.exceptions = typeof WebAssembly.Exception !== 'undefined';

    } catch (error) {
      console.warn('Error checking WASM 2.0 features:', error);
    }
  ...
  getDiagnostics() {
    return {
      ...
      wasm2Features: this.checkWasm2Features()
    };
  }
}
